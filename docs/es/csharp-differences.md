...
- Las entradas que no se modifican conservan los bytes exactos de su fuente, incluso cuando cambian otras entradas: se aplica automáticamente la división en bloques de bytes por entrada.

Consulta [las garantías de Fidelity](fidelity.md).

## Validación

El `Validator` de C# ejecuta una pasada de RELAX NG y devuelve los primeros errores en forma de cadenas de caracteres. sil-lift genera un flujo estructurado de «Problemas», cada uno de los cuales incluye el archivo, la entrada y la línea a los que se refiere, y su capa de esquema presenta tres divergencias deliberadas:
...
- **Las comparaciones de identificadores de rango están normalizadas según Unicode** (NFC), ya que la exportación de FLEx no siempre era coherente internamente: se normaliza a NFC al salir, pero algunas entradas de `.lift-ranges` solían saltarse ese paso y generar NFD, por lo que un `id` de un elemento de rango de `grammatical-info` o `lexical-relation` puede estar en NFD, mientras que sus etiquetas, el atributo `parent` de ese mismo elemento y el valor `.lift` que hace referencia a él están todos en NFC. La normalización se aplica únicamente a la comparación: sil-lift nunca reescribe los identificadores, y las referencias que solo se resuelven tras la normalización se señalan como advertencias de `normalization-mismatch` —una comprobación que no tiene equivalente en C#—, de modo que la división de la codificación sigue siendo visible para cualquiera cuyas comparaciones sean exactas.
...
- `SynchronicMerger` (fusión de archivos de actualización de LIFT): la idea de la división en bloques de bytes se mantiene en la capa de fidelidad, pero la fusión no.
