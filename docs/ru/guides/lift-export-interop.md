...
- **Напишите:** `Lexicon.save_zip("out.zip", wrap_folder="MyDict")` упаковывает файл `.lift`, его файлы `.lift-ranges`, а также все остальные файлы из исходной папки (медиафайлы, `WritingSystems/`, `consent/`, ...) в архив ZIP.
  ...
  - Записи, содержание которых изменилось после загрузки, при выводе помечаются новой датой `dateModified`, точно так же, как при вызове `save()`: пакет служит основой для синхронизации импортирующего инструмента, поэтому устаревшая дата в этом случае приводит к тому, что обновленный словарь выглядит как неизменённый. `stamp=False` и `when=` здесь работают одинаково.
