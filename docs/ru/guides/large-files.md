...
- Кроме того, она не создаёт временные метки, как это делает [`Lexicon.save()`](../fidelity.md#saving-an-edited-document). Запись формируется с указанием дат, которые она содержит: потоковый пропуск никогда не видит документ в его исходном виде, поэтому он не может определить, какие записи вы изменили. Вручную установите значение `entry.date_modified` для тех записей, которые перезаписываются в ходе этого прохода.
