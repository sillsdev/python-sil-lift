...
pron.forms["en"] = "Speaker: Ana"  # Конвенция обозначения говорящего в проекте Combine
...
# Проверить документ в том виде, в каком он есть, прежде чем записывать его на диск.
...
<entry id="kanga" guid="6b9e7c2a-3f4d-4a1b-8c5e-2d9f0a1b2c3d" dateCreated="2026-09-02T19:19:17Z" dateModified="2026-09-02T19:19:17Z">
  ...
  <sense id="kanga_s1">
    ...
    <definition>
      <form lang="en">
        <text>домашняя птица, которую разводят ради яиц и мяса</text>
      </form>
    </definition>
    <example>
      <form lang="seh">
...
- Значения `dateCreated`/`dateModified` в приведённом выше выводе отсутствуют в скрипте: функция `save()` присвоила им дату и время своего выполнения, поскольку запись, созданная впервые, не содержит собственной даты. Параметр `when=` указывает этот момент вместо считывания времени с часов, что обеспечивает воспроизводимость с точностью до байта сгенерированного экспорта при сравнении в задании CI; параметр `stamp=False` не записывает даты вообще. Остальную часть контракта см. в разделе [Сгенерированные временные метки](../fidelity.md#generated-timestamps).
- Функция `lex.iter_problems()` проверяет документ, хранящийся в памяти, до того, как какие-либо данные попадут на диск; в данном случае он исправен. Поскольку для лексикона пока не создана папка, проверки наличия медиафайлов и ссылок на сопутствующие ресурсы пропускаются — запустите команду [`sil-lift validate`](cli.md) на сохраненном выводе (или с параметром `--no-check-media`), как только аудио- и фотофайлы будут размещены в соответствующих папках.
