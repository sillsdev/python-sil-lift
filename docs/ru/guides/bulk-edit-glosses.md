...
if errors:
    for problem in errors:
        print(problem)
    sys.exit(f"прерывание: {len(errors)} ошибка(и) валидации, ничего не сохранено")

lex.save()
print(f"отредактировано {edited_glosses} глосса(рий) в {len(touched_entries)} записи(ях)")
```
