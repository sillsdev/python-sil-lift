...
if errors:
    for problem in errors:
        print(problem)
    sys.exit(f"прерывание: {len(errors)} ошибок валидации, ничего не сохранено")

lex.save()
print(f"отредактированы {edited_glosses} глоссы в {len(changed)} записях")
```

Несколько моментов, на которые стоит обратить внимание:

- `entry.all_senses()` возвращает все значения _и подзначения_, в порядке глубинного обхода по порядку их появления в документе.
  - `entry.senses` содержит только элементы верхнего уровня, поэтому при массовом редактировании, проходящем по этому списку, любые глоссы, вложенные под подзначениями, будут незаметно пропущены.
...
- Функция `lex.save()` устанавливает новое значение `dateModified` именно для тех записей, которые были изменены этим скриптом, а также заполняет поле `dateCreated`, если оно было пустым. Без этого лексикон, переписанный данным скриптом, будет выглядеть неизменённым для FieldWorks или для импорта LIFT в The Combine, поскольку обе эти программы сами определяют, какие данные из данного атрибута следует обновить — и они помечают только `<entry>`, независимо от глубины внесённых изменений: именно запись, содержащую отредактированное пояснение подзначения, а не само значение.
