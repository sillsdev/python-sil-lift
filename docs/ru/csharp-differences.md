...
- неизмененные записи сохраняют свои исходные байты в точности, даже если другие записи изменяются — это тот же принцип разбиения на байтовые блоки для каждой записи, который использует Chorus, и который применяется автоматически.
