...
- **Сравнения идентификаторов диапазонов проводятся в нормализованном формате Unicode** (NFC), поскольку экспорт из FLEx не всегда был внутренне согласован: при выводе данные нормализуются в формат NFC, но некоторые записи `.lift-ranges` ранее обходили этот этап и выдавали данные в формате NFD, поэтому `id` элемента диапазона `grammatical-info` или `lexical-relation` может быть в формате NFD, в то время как его метки, атрибут `parent` того же элемента и значение `.lift`, ссылающееся на него, все находятся в формате NFC. Нормализация касается исключительно сравнения: sil-lift никогда не перезаписывает идентификаторы, а ссылки, которые разрешаются только после нормализации, фиксируются как предупреждения `normalization-mismatch` — проверка, не имеющая аналога в C#, — поэтому различия в кодировке остаются заметными для всех, чьи собственные сравнения являются точными.
