...
- To ask whether the document changed *at all* — not just its entries — use `lex.changes()`. It also covers the header, the root element, and every `.lift-ranges` companion, and is truthy exactly when `save()` would not reproduce the source bytes, which makes `if not lex.changes(): ...` the right way to skip an unnecessary write. `changed_entries()` alone would miss a header-only or ranges-only edit.
