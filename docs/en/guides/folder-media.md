...
Companion discovery handles the real world: a `range/@href` that points at an existing file is used; FLEx's dangling absolute `file://C:/...` hrefs fall back to the href's basename next to the `.lift`; and the conventional `<name>.lift-ranges` sibling is picked up even when nothing references it.
