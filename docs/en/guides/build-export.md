...
- The `dateCreated`/`dateModified` in the output above are not in the script. `save()` stamped them with the moment it ran: an entry written for the first time carries no date of its own, and the tools that import LIFT decide what to update from `dateModified`. Two knobs control it — `when=` supplies the moment instead of reading the clock (timezone-aware, normalized to UTC whole seconds), which is what makes a generated export byte-reproducible for a CI job to diff, and `stamp=False` writes no dates at all. A date you set yourself is left alone either way, and nothing below `<entry>` is ever stamped.
