...
- `lex.save()` versieht genau die Einträge, die dieses Skript geändert hat, mit einem neuen `dateModified`-Zeitstempel und füllt dabei `dateCreated` aus, sofern dieses Feld leer war. Ohne diese Angabe würde ein von diesem Skript überarbeitetes Lexikon für FieldWorks oder den LIFT-Import von The Combine unverändert erscheinen, da beide Programme entscheiden, welche Daten aus diesem Attribut aktualisiert werden sollen – und es wird ausschließlich `<entry>` markiert, unabhängig davon, wie tiefgreifend die Bearbeitung war: der Eintrag, der die bearbeitete Unterbedeutung enthält, nicht die Bedeutung selbst.
