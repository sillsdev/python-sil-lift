...
- 它也不会像 [`Lexicon.save()`](../fidelity.md#saving-an-edited-document) 那样生成时间戳。每条记录都带有其对应的日期：流式读取器永远无法看到文档的原始状态，因此无法判断您修改了哪些记录。对于本次处理中被重写的条目，请自行设置 `entry.date_modified`。
