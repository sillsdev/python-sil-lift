...
- `lex.save()` 会在本脚本修改过的条目上标记一个新的 `dateModified`，并将 `dateCreated` 字段（若为空）填上相应值。如果没有它，该脚本重写的词汇表在 FieldWorks 或 The Combine 的 LIFT 导入功能中看起来会像未被修改过一样——这两者都会根据该属性决定更新哪些内容——而且它只会标记 `<entry>`，无论编辑涉及多深：标记的是包含已编辑子义释义的条目，而不是该词义本身。
