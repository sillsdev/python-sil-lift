...
- le scanner d'octets et l'analyseur syntaxique ne s'accordent pas sur la structure de premier niveau du document — par exemple, un deuxième `<header>` non conforme à la spécification, que l'analyseur syntaxique ne conserve qu'une seule fois (le scanner se montre délibérément méfiant : le moindre doute l'amène à ne capturer aucun octet source), ou
...
- Les commentaires situés _à l'intérieur_ d'une exécution `<text>` sont conservés, mais déplacés à côté de l'exécution, et non à leur position exacte en caractères.
