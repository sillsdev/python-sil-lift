...
- **Les comparaisons d'identifiants de plage sont normalisées selon la norme Unicode** (NFC), car l'exportation de FLEx n'était pas toujours cohérente en interne : elle effectue une normalisation vers NFC lors de la sortie, mais certaines écritures `.lift-ranges` contournaient autrefois cette étape et généraient du NFD, de sorte qu’un `id` d’élément de plage `grammatical-info` ou `lexical-relation` peut être en NFD tandis que ses étiquettes, l’attribut `parent` de ce même élément et la valeur `.lift` qui y fait référence sont tous en NFC. La normalisation ne concerne que la comparaison : sil-lift ne réécrit jamais les identifiants, et les références qui ne peuvent être résolues qu’après normalisation sont signalées par des avertissements de type « `normalization-mismatch` » — une vérification qui n’a pas d’équivalent en C# —, de sorte que la division d’encodage reste visible pour tous ceux dont les comparaisons sont exactes.
