...
- **記述例：** `Lexicon.save_zip("out.zip", wrap_folder="MyDict")` とすると、`.lift`、その `.lift-ranges`、およびソースフォルダ内のその他のすべてのファイル（media、`WritingSystems/`、`consent/` など）がまとめてパッケージ化されます。 zipファイルにまとめる。
  ...
  - 読み込み後に内容が変更されたエントリには、`save()` の場合と同様に、書き出し時に新しい `dateModified` が付与されます。パッケージはインポート元ツールが照合を行う対象であるため、そこに古い日付が残っていると、更新された辞書が変更されていないように見えてしまうのです。 `stamp=False` と `when=` は、ここでは同じように動作します。
