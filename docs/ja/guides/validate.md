...
3. **文法では表現できない意味論的チェック** — 計12項目、それぞれ1つのコード。

## 問題コード

各検出結果には、それがどのレイヤーで生成されたかにかかわらず、これらのいずれかが含まれます。`schema` と `uri-not-rfc` はスキーマ・レイヤーに由来し、残りの12個はセマンティックチェックによるものです。文字列はサポートされているインターフェースです。`--strict` を指定すると、すべての警告がエラーとして扱われます。
...
| `unreadable-ranges-file` | 警告  | 存在はするが、読み取り可能な範囲ドキュメントではないコンパニオン候補                       |
...
3つのコンパニオン・フォルダ・コード（`ambiguous-ranges-file`、`dangling-ranges-href`、および`unreadable-ranges-file`）は、コンパニオン検出が実行された場合にのみ報告されます。 `resolve_ranges=False` を指定して読み込むと、コンパニオンがスコープ外となるため、いずれも報告されません。その後、`add_ranges_file()` を使ってコンパニオンを追加しても、これらのコードが報告対象とするフォルダは読み込まれないため、コンパニオンは再び報告対象にはなりません。 `missing-media` には影響がありません。メディアはモデルに解決されることが決してないため、除外されたものはありませんでした。
