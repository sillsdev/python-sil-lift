...
- また、[`Lexicon.save()`](../fidelity.md#saving-an-edited-document) のようにタイムスタンプを生成することもありません。エントリは、そこに記載された日付とともに記録されます。ストリーミングパスでは、ドキュメントの元の状態を確認することはできないため、どのエントリが変更されたかを特定することはできません。このパスで書き換えられる項目については、`entry.date_modified` を手動で設定してください。
